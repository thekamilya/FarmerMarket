...
class GetSoldProductsUseCase @Inject constructor(
    ...
    operator fun invoke(status: String): Flow<Resource<SoldProductsDto>> = flow {
        try {
            emit(Resource.Loading())
            val products = repository.getSoldProducts(status)
            emit(Resource.Success<SoldProductsDto>(products))
            Log.d("kama", products.toString())
        } catch (e: HttpException) {
            emit(Resource.Error<SoldProductsDto>(e.localizedMessage ?: "An unexpected error occured"))
            Log.d("kama", e.message.toString())
        } catch (e: IOException) {
            Log.d("kama", e.message.toString())
            emit(Resource.Error<SoldProductsDto>(e.message.toString()))
        }
    }
}